...
#   (f) PI-33 smoke: `xdpmf-exporter --version` reports `xdpmf-exporter 0.7.0`
#       (HK-8 version bump per §5.31 — PI-8-3.4b extends PI-33 to 0.7.0).
#
...
fail=0
if [[ "${ver}" != "xdpmf-exporter 0.7.0" ]]; then
    echo "FAIL[f]: expected --version output 'xdpmf-exporter 0.7.0', got '${ver}'" >&2
    fail=1
fi
