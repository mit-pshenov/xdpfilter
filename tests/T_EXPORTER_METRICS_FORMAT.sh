...
#   (f) PI-33 smoke: `xdpmf-exporter --version` reports `xdpmf-exporter 0.6.1`
#       (HK-8 version bump per §5.30 — PI-8-3.4.5 extends PI-33 to 0.6.1).
#
...
fail=0
if [[ "${ver}" != "xdpmf-exporter 0.6.1" ]]; then
    echo "FAIL[f]: expected --version output 'xdpmf-exporter 0.6.1', got '${ver}'" >&2
    fail=1
fi
