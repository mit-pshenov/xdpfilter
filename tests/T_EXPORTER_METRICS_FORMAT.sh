...
#   (f) PI-33 smoke: `xdpmf-exporter --version` reports `xdpmf-exporter 0.11.0`
#       (PI-8-3.4d version bump per §5.35 — extends PI-33 to 0.11.0;
...
fail=0
if [[ "${ver}" != "xdpmf-exporter 0.11.0" ]]; then
    echo "FAIL[f]: expected --version output 'xdpmf-exporter 0.11.0', got '${ver}'" >&2
    fail=1
fi
