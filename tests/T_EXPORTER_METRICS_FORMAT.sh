...
#   (f) PI-33 smoke: `xdpmf-exporter --version` reports `xdpmf-exporter 0.12.0`
#       (PI-mvp-4.4-VERSION bump per §5.44 — extends PI-33 to 0.12.0;
...
fail=0
if [[ "${ver}" != "xdpmf-exporter 0.12.0" ]]; then
    echo "FAIL[f]: expected --version output 'xdpmf-exporter 0.12.0', got '${ver}'" >&2
    fail=1
fi
