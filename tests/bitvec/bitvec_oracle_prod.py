...
HASH + `loader.cpp`'s `close_prefixes()`), so that any disagreement localises
a closure / wildcard / range / first-match bug in the datapath rather than
masking it (§5.43 §6.61, §5.44 §6.66).

TWO independent rule tables (dual-table — both transcribed BY HAND from their
...
independence precedent):
  ...
  * RULES_AND4  (4-axis) — transcribed from tests/fixtures/config_valid_and4.yaml.
                ...
                --ruleset and4 --proto P --dport N.

If you edit either fixture you MUST edit the matching table here.

Semantics (§5.43 §6.61 / §5.44 §6.66 / DataStructures):
...
def main():
    ap = argparse.ArgumentParser(
        ...
        description="independent production AND first-match classifier "
                    "(§5.43 2-axis / §5.44 4-axis)")
    ap.add_argument("--ruleset", choices=("and", "and4"), default="and",
    ...
    ap.add_argument("--proto", type=_proto_arg,
                    help="IP protocol (tcp/udp/icmp or number); and4 only")
    ap.add_argument("--dport", type=int, default=0,
                    help="L4 dest port; and4 only (ignored for icmp)")
    args = ap.parse_args()

    if args.ruleset == "and4":
