...
def main():
    ...
    ap.add_argument("--ruleset", choices=("and", "and4", "and5"), default="and",
    ...
    ap.add_argument("--proto", type=_proto_arg,
                    help="IP protocol (tcp/udp/icmp or number); and4/and5 only")
    ap.add_argument("--dport", type=int, default=0,
                    help="L4 dest port; and4/and5 only (ignored for icmp)")
    ap.add_argument("--vlan", type=int, default=None,
                    help="outer 802.1Q VID; and5 only (omit ⇒ untagged frame, "
                         "has_vlan=0)")
    args = ap.parse_args()

    if args.ruleset == "and5":
