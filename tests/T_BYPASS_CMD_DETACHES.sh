...
#
# `xdpmacfilter bypass --iface X --unsafe --reason Y` detaches XDP + emits
...
#   3. Run `xdpmacfilter bypass --iface IFACE_A --unsafe --reason "T_BYPASS_test"`
...
#       '^xdpmacfilter: BYPASS activated on <IFACE_A> by uid=[0-9]+ reason="T_BYPASS_test"$'
