/*
 * mac_filter_alt.bpf.c — tag-mismatch fixture for T_ATTACH_TAG_MISMATCH
 * (design §6.14, MVP-2 Sec).
 *
 * Same `SEC()` function name as the real prog (`mac_filter_prog`), so the
 ...
 * from src/bpf/mac_filter.bpf.c's built object.
 *
 * Load-bearing constraints (per design §6.14):
 *   - Function name MUST be `mac_filter_prog` (identical to real prog).
 ...
 *
 * Built via `add_bpf_object(mac_filter_alt …)` in tests/CMakeLists.txt;
