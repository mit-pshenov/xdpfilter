...
#
# Standalone verifier-load of the PRODUCTION mac_filter.bpf.o (the SHIPPED
...
#   - points at ${BUILD_DIR}/mac_filter.bpf.o (the production object), and
...
#                complexity/stack-budget regression in mac_filter.bpf.c) makes
...
# absent, mac_filter.bpf.o not built, or passwordless sudo unavailable
...
# mac_filter.bpf.o → recursive find under ${BUILD_DIR}.
find_prod_obj() {
    if [[ -n "${PROD_BPF_OBJ:-}" && -f "${PROD_BPF_OBJ}" ]]; then
        printf '%s\n' "${PROD_BPF_OBJ}"; return 0
    fi
    if [[ -f "${BUILD_DIR}/mac_filter.bpf.o" ]]; then
        printf '%s\n' "${BUILD_DIR}/mac_filter.bpf.o"; return 0
    fi
    local found
    found=$(find "${BUILD_DIR}" -maxdepth 5 -type f -name 'mac_filter.bpf.o' 2>/dev/null | head -1 || true)
    [[ -n "${found}" ]] && { printf '%s\n' "${found}"; return 0; }
    return 1
}
...
if [[ -z "${PROD_OBJ}" ]]; then
    echo "SKIP: production mac_filter.bpf.o not found under ${BUILD_DIR}" >&2
...
if [[ "${rc}" -eq 0 ]]; then
    ...
    echo "FAIL: verifier REJECTED the production mac_filter.bpf.o (rc=${rc})" >&2
    ...
    echo "      src/bpf/mac_filter.bpf.c. See verifier log above." >&2
    fail=1
fi

[[ "${fail}" == 0 ]] && echo "PASS: T_PROD_VERIFIER_LOAD (production mac_filter.bpf.o loads+verifies on $(uname -r))"
exit "${fail}"
