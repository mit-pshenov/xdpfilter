...
## §MVP-3.4 Open Question #13 RESOLUTION

(Synthesis committed via `/mint-hld` round 2026-05-24, brief at `mint/design-brief.md`. Architects: HASH, ARRAY, T-contrarian. Verdict: `pass` round 1.)

> **Human-gate decision (2026-05-24)**: per-rule counters **DEFERRED** to MVP-3.4b (Option 1, T.1). MVP-3.4 ships exporter binary for global `stats` PERCPU_ARRAY + manual bypass primitive + `rules`+`action_table` BPF skeleton — no per-rule counter wiring, no inner-map-value extension this cycle. The map-type pick (HASH vs ARRAY) is preserved below as documented decision space; when MVP-3.4b is scoped, this resolution serves as the survey input and Option 2 (Sparse-direct-bounded ARRAY) is the default if shipping per-rule counters then. Rationale: (a) MVP-3.4 already at upper edge of slice budget (2-3 cycles vs §5.28 ≈1.5-cycle norm); (b) no named operator awaits per-rule visibility today; (c) `/metrics` for global counters is the cycle-1 user value (currently zero observability surface); (d) PI-13-3.1 adjudication on inner-value extension can be done deliberately before MVP-3.4b rather than under slice pressure. Open Q #13 status: **technical decision space surveyed; ship/defer answered = defer; map-type pick conditional on Open Q #13's re-entry at MVP-3.4b scoping**.

> **Process flag — register before reading the resolution.** The brief BODY at lines 35-40 describes the schema as "sparse-id-permissive (operator could write `id: 1`, `id: 5`, `id: 100`)". **This is factually wrong as written.** Per `mint/design.md` §5.26 schema rule 3 (line 3908-3910), `id MUST be ∈ [0, XDPMF_ALLOWLIST_MAX - 1] = [0, 63]`; `id: 100` is rejected by the validator today. ARRAY architect caught this (architect-ARRAY.md line 13); T architect caught it and treated it as the load-bearing observation of the round (T.2, architect-T.md line 323-341); **HASH architect did not catch it and inherited the brief framing as the foundational premise of HASH.1's "zero schema change" argument** (architect-HASH.md line 15-16). This is the silent-inheritance-pattern recurrence explicitly named as a process anti-pattern at this document's Hidden Assumptions #1 (line 397). Verified independently by synthesizer: `mint/design.md:3908-3910` and `src/common/mac_filter.h:67` (XDPMF_ALLOWLIST_MAX == 64). The synthesis below treats the corrected premise — `id` is **bounded-sparse** (∈ [0,63], gaps permitted, ≤64 rules), NOT unbounded-sparse — as load-bearing throughout.

### Convergence (where N architects agree)

- **(HASH, ARRAY): the existing `stats` PERCPU_ARRAY `bump_stat` idiom is the byte-equivalent template for per-rule counter wiring.** HASH.1 explicitly says "the per-rule map joins the same idiom" (HASH §Phase 1 takeaway, line 13); ARRAY.1 says "Identical shape to existing `bump_stat`; verifier sees one lookup + one null-check + one increment" (ARRAY §ARRAY.1 How-it-works). Whichever map type wins, the datapath shape is `lookup_elem → if (v) *v += 1` — reviewer pattern-recognition cost is identical.

- **(HASH, ARRAY, T): the inner-map-value shape — how `rule_id` reaches the datapath at bump-time — is a co-equal load-bearing decision that this round does NOT resolve.** HASH.1 OQ #1 names it ("`__u8` presence-flag → `__u32 rule_id`"); ARRAY.1 OQ #1 and ARRAY.1 §What-it-costs name it ("extending `u8 present` → `struct{u8 present; u32 rule_id;}`"); T.5 owns it explicitly as "the real load-bearing decision is the inner-map-value shape, not the map type" (architect-T.md line 223-318). Resolving Open Q #13 without resolving S-1 (extend-inner-value) vs S-2 (parallel-sidecar-map) is a fake unblock — the MVP-3.4 implementation stays gated.

- **(HASH, ARRAY, T): HASH.8 (intern-hybrid) and ARRAY.6 (two-map shadow) are mechanically equivalent at the BPF surface for migration purposes.** HASH.8 stores `__u16 internal_id` in BPF with a userspace intern table mapping operator-id↔internal-id. ARRAY.6 stores counters in PERCPU_ARRAY[64] keyed by rule_id with metadata (including operator's id and any future `name:`) in a userspace JSON sidecar. Both decouple operator-id allocation from kernel-side key shape; both reduce HASH↔ARRAY migration to "swap one BPF map type declaration, keep userspace mapping". HASH architect acknowledges this in cross-lens hooks ("makes our two recommendations nearly mechanically equivalent at the BPF surface"); T calls out the convergence as T.8.

- **(ARRAY, T): the brief's premise is wrong and the corrected premise (`id ∈ [0,63]`, bounded-sparse) makes ARRAY the natural fit under ALL plausible allocation policies.** ARRAY architect catches the factual error (line 13); T's T.2 (PREMISE-BUST) and T.3 (REFRAME) explicitly enumerate the three viable allocation policies (sparse-direct-bounded / dense-auto / intern-via-name) and show ARRAY is the mechanical map-type consequence in all three. HASH-camp's argument exists only if `id ∈ [0,63]` cap is **lifted** — which is a larger separate decision (§5.26 schema rule 3 amendment) not scoped in Open Q #13.

- **(ARRAY, T): the 64-rule cap is shipped, contractual, and worth a deliberate constant alias.** ARRAY notes §5.26 rule 3 has been live across 11 mint-dev cycles (architect-ARRAY.md line 22); T.9 proposes `#define XDPMF_RULE_COUNTERS_MAX XDPMF_ALLOWLIST_MAX` in `src/common/mac_filter.h` to lock in "you cannot have more counters than rules". Both treat the cap as "feature, not defect" per arch-v2.md risk-register line 337.

- **(HASH, ARRAY, T): migration-cost-between-map-types is honestly 2-3 cycles, not 1 — driven by the inner-map-value shape change rather than the map declaration.** HASH OQ #5 says "~1 cycle, in-place" — best-case; ARRAY OQ #4 acknowledges PI-13-3.1 inner-shape risk; T.6 provides the honest accounting (1.75-2.4 cycles cumulative). The decision is NOT cheaply reversible; synthesizer should NOT pick on "we can flip later" comfort.

### Divergence (where architects substantively disagree)

- **(HASH vs ARRAY+T): is the brief's "sparse-permissive" framing load-bearing as stated, or factually wrong?** HASH treats the brief BODY (line 35-40) as the foundational input ("the load-bearing sparse-permissive constraint is honored byte-for-byte" — architect-HASH.md HASH.1 What-it-gives, line 108). ARRAY+T re-test against §5.26 rule 3 and find the brief misrepresents the shipped schema. **Implication of resolving:** if HASH's framing stands, HASH-camp arguments are valid and a HASH choice is defensible; if ARRAY+T's correction stands (which the synthesizer verified independently — it does), the HASH camp's case collapses to "we would like to lift §5.26 rule 3" — a different, larger question.

- **(HASH vs ARRAY): should the schema EVOLVE in this round (named rules) or stay still?** HASH.1, HASH.4, and HASH.8 all preserve operator-facing schema; ARRAY.1 and ARRAY.6 also preserve it; **ARRAY.3** is the schema-evolving move (new REQUIRED `name:` field, schema-v2, PI-17 Jinja2 template touch, ~6 fixture edits, stable rule identity across renumber). HASH explicitly rejects schema-breaking (HASH.7 enumerated then deselected as "gratuitous schema break"). **Implication of resolving:** if no, ARRAY.1 / ARRAY.6 / HASH.1 are the candidate set; if yes, ARRAY.3 wins and pays its own cost.

- **(HASH.4 vs ARRAY+T.10): should composite-key counter taxonomy (per-rule × per-axis × per-verdict in ONE map) land in MVP-3.4?** HASH.4 argues this is the "expressive/future-proof corner" and pays only ~30 LOC exporter complexity. ARRAY does not propose any analogue. T.10 explicitly rejects: "pre-commits to a counter taxonomy that has had zero operator validation". HASH's own OQ #1 admits the migration to HASH.4 from HASH.1 is in-place. **Implication of resolving:** HASH.4 belongs in MVP-3.5+ if anywhere; first slice should be simple per-rule.

- **(T vs HASH+ARRAY): should per-rule counters ship in MVP-3.4 at all, or defer to MVP-3.4b?** T.1 (DEFER) is the only "don't-ship" candidate; both HASH and ARRAY assume the feature ships and design accordingly. T notes MVP-3.4 is currently the largest single slice on the roadmap (2-3 cycles vs team's recent ~1.5-cycle norm at §5.28) and that per-rule visibility has no named operator demand right now. **Resolved at human gate 2026-05-24: defer.**

- **(HASH.1 implicit vs T.6 explicit): how reversible is the choice if benchmarks demand the other map type later?** HASH frames migration as ~1 cycle; HASH.8 actively markets itself as "the migration affordance no other approach provides cleanly". T.6 puts honest accounting at 1.75-2.4 cycles (inner-map-value PI-13 break is the cost driver, not the map declaration). **Implication of resolving:** if T.6 is right (synthesizer agrees — the inner-value cost dominates), the "decouple via HASH.8 / ARRAY.6" optionality argument shrinks, and ARRAY.1 + S-1 becomes cleaner because it pays the inner-value cost ONCE explicitly rather than maintaining the intern/sidecar layer perpetually.

### Composite directions (cross-lens combinations)

#### Option 1 — "Honest defer" (T-aligned, minimum-ship) ← human-gate pick

- **Composition**: T.1 (DEFER per-rule counters out of MVP-3.4) + no map-type pick this round + Option 2 or 3 becomes the candidate set when this question is re-asked at MVP-3.4b.
- **First slice scope**: MVP-3.4 = `xdpmf-exporter` binary serving the existing global `stats` PERCPU_ARRAY[STAT_MAX=4] via `/metrics` + manual bypass primitive + `rules`+`action_table` BPF skeleton (B.2 partial, no counter wiring). Per-rule counter map + inner-value shape decision deferred to MVP-3.4b.
- **Risk profile**: **low** — no new BPF maps, no inner-shape change, no PI-13 adjudication, no schema touch. Largest risk is whether `rules`+`action_table` skeleton can be added without inner-value extension (T.1 OQ #2).
- **User value cycle 1**: `/metrics` surface NOW for global PASS/DROP_DENY/DROP_MALFORMED/PASS_CIDR counters (currently no surface AT ALL); manual bypass primitive available.
- **Costs**: TTFW ≤1.5 cycles (matches team's §5.28 norm); per-rule slice slips 1 cycle; Open Q #13 re-asked at MVP-3.4b with same architects (~30 min orchestration cost); fragments the MVP-3.4 "observability" narrative across two slices.
- **Preserves**: §5.26 schema rule 3 untouched; PI-13-3.1 inner-shape contract untouched; PI-17 Ansible template untouched; existing 11 mint-dev cycles' artefacts byte-equivalent.
- **Open Qs for this option**:
  1. Does the `rules`+`action_table` BPF skeleton REQUIRE inner-value extension regardless of per-rule counters? If yes (and the MVP-3.4 brief assumes it), the PI-13 cost is paid anyway and Option 1's value shrinks.
  2. Is there a named operator awaiting per-rule visibility in MVP-3.4? Current arch-v2.md says no; if yes, Option 1 is wrong-shape.
  3. Should manual bypass ALSO defer (T.7 double-defer) to make MVP-3.4 a pure-observability slice?

#### Option 2 — "Sparse-direct-bounded ARRAY" (synthesizer default if shipping; MVP-3.4b candidate)

- **Composition**: ARRAY.1 (status-quo-wide PERCPU_ARRAY[64], operator's `id ∈ [0,63]` is the BPF key directly, sparse-but-bounded usage) + T.5 S-1 (extend inner-map value `__u8 → struct{__u8 present; __u32 rule_id;}` to carry rule_id to datapath) + T.9 (alias `XDPMF_RULE_COUNTERS_MAX = XDPMF_ALLOWLIST_MAX = 64`) + small `<pin_dir>/rule_index.json` sidecar (loader-written; optional, for human-readable labels).
- **First slice scope**: PERCPU_ARRAY[64] of u64 counter map declared in `mac_filter.bpf.c`; `bump_rule(__u32 rule_id)` inline helper added beside existing `bump_stat`; inner allowlist value extended to `struct allow_entry { __u8 present; __u8 _pad[3]; __u32 rule_id; }` (both MAC HASH and CIDR LPM_TRIE — symmetric); loader writes apply-time sidecar JSON; exporter sums per-CPU per slot, joins sidecar for labels, emits `xdpfilter_rule_match_total{iface, rule_id}`.
- **Risk profile**: **medium** — gated on PI-13-3.1 adjudication (T.5 OQ #1): does extending inner value from `__u8` to `struct{__u8 present; ...}` PASS as additive (recommended interpretation: yes — `present` stays at offset 0 with same semantics) or VIOLATE as byte-shape break? If VIOLATE, fall back to Option 3.
- **User value cycle 1**: per-rule counter visible via `/metrics`; sparse operator-assigned `id` survives end-to-end without intermediate translation.
- **Costs**: ~5 fixture touches (PI-13 inner-shape migration); CIDR LPM_TRIE inner-value sister change (T.5 OQ #3 — symmetric); +16 KiB total BPF map memory (64 × 8 B × 32 CPUs — rounding error); inner-map per-entry size 1 B → 8 B (~1 KiB across both A/B inner maps).
- **Preserves**: §5.26 schema rule 3 invariant; bounded-sparse semantic; verifier-trivial datapath (joins existing `bump_stat` byte-equivalent); no PI-17 Jinja2 touch.
- **Open Qs for this option**:
  1. PI-13-3.1 adjudication on inner-value extension — additive PASS or byte-shape break VIOLATE? (Needs reviewer ruling before MVP-3.4b starts.)
  2. Sidecar JSON vs no-sidecar: do we need a `rule_index.json` for human-readable labels, or is `rule_id="3"` sufficient for cycle 1?
  3. Does counter-survive-across-apply (Prometheus-counter semantic) need to be a documented guarantee, or implementation-detail? (HASH.1 OQ #3, applies equally here.)

#### Option 3 — "Decouple via two-map shadow" (HASH.8 ≡ ARRAY.6 convergence, PI-13-safe fallback)

- **Composition**: ARRAY.6 (PERCPU_ARRAY[64] of u64 + userspace JSON sidecar for metadata) + T.5 S-2 (parallel BPF sidecar map `mac → rule_id` HASH and `cidr → rule_id` LPM_TRIE; inner allowlist value untouched at `__u8`) + T.9 (cap alias). Equivalent to HASH.8 with HASH→ARRAY map-type substitution; functionally identical decoupling.
- **First slice scope**: PERCPU_ARRAY[64] of u64 counter map + new `mac_to_rule_id` BPF HASH + new `cidr_to_rule_id` BPF LPM_TRIE; datapath does one extra lookup per matching packet to fetch rule_id; loader writes sidecar JSON; exporter reads BPF + JSON, drops orphan rule_ids in skew window.
- **Risk profile**: **medium** — extra per-packet lookup (~10-15 ns at matching sites), two-file consistency window between BPF map and JSON (sub-millisecond, observable to scrape only if it lands exactly within the window — at 15s Prometheus cadence: negligible).
- **User value cycle 1**: per-rule counters via `/metrics`; PI-13-3.1 inner-shape invariant untouched; schema invariance.
- **Costs**: extra BPF maps (2 — MAC sidecar + CIDR sidecar); extra per-packet lookup tax; exporter must tolerate sidecar-bpf skew; "rule_id in counter map but not in metadata" race-window output (drop and reconcile next scrape).
- **Preserves**: PI-13-3.1 inner-shape; §5.26 schema; PI-17 Jinja2; schema-evolving freedom for later rounds (sidecar absorbs whatever shape; future ARRAY.3-style `name:` is a sidecar-only change).
- **Open Qs for this option**:
  1. Is the ~10-15 ns/packet extra lookup tax in any acceptance budget? (Probably not load-bearing at MVP-3.4; needs noting.)
  2. Should the sidecar be a JSON file or a pinned BPF map of structs (ARRAY.6 OQ #1)? JSON for v1.
  3. Does the sidecar-bpf consistency window need an explicit ctest (`T_APPLY_SIDECAR_RACE_FREE` per ARRAY.6 cross-lens hook)?

#### Option 4 — "Schema-evolve to named rules" (ARRAY.3, forward-looking)

- **Composition**: ARRAY.3 (schema-v2 adds REQUIRED `name:` per rule; loader sorts and assigns internal dense `rule_id 0..N-1`; operator's `id:` retained as Prometheus label only) + T.5 S-1 (inner-value carries internal rule_id) + T.9 (cap alias).
- **First slice scope**: schema-v2 definition + `name:` grammar validator (regex, length, uniqueness) + v1→v2 auto-migration (synthesize `name="rule_${id}"` for v1 configs) + loader allocator (stable sort by name → internal rule_id) + PERCPU_ARRAY[64] + inner-value extension + Prometheus labels by name + PI-17 Jinja2 template emits `schema_version: 2`.
- **Risk profile**: **high** — schema breaking-ish change; ~6 fixture edits + 1 Jinja2 template edit + validator extension; PI-13-3.1 adjudication AND PI-17 successor-invariant negotiation BOTH required.
- **User value cycle 1**: stable rule identity across renumber (operator deletes rule, others' counters survive); human-readable Prometheus labels (`rule="web-pass"` vs `rule="3"`); forward-fit if MVP-3.5 JSON-log slice ships and operators want `rule="web-pass"` in log entries.
- **Costs**: ~0.5-1 cycle extra vs Option 2; new operator-facing field grammar; PI-17 successor invariant adjudication (new `schema_version: 2` literal at template line 1); cognitive load (operators read renamed schema doc).
- **Preserves**: 64-rule cap; operator's `id:` retained as label (back-compat); HASH-migration path stays cheap (name → key works for HASH too).
- **Open Qs for this option**:
  1. Does v1→v2 schema bump count as "breaking" requiring operator-gated rollout, or is auto-synthesis-of-`name` enough back-compat to ship silently (ARRAY.3 OQ #1)?
  2. Sort key: ASCII (recommended), hash, YAML-source-order? (ARRAY.3 OQ #2.)
  3. Is there operator demand for named rules NOW, or is this premature optimization? (Synthesizer note: no operator demand named in arch-v2.md or MVP-3.3 retrospective.)

#### Option 5 — "HASH.1 as-spec" (steel-man, NOT recommended)

- **Composition**: HASH.1 (PERCPU_HASH keyed by operator `id`, `max_entries=256`) + T.5 S-1 (inner-value carries rule_id) + tacit non-amendment of §5.26 rule 3.
- **First slice scope**: PERCPU_HASH of `__u32 → __u64`, max_entries=256, pinned; bump_rule(rule_id) datapath; inner-value extension; exporter dumps and sums per key.
- **Risk profile**: **high** — load-bearing argument depends on the brief's misframed "sparse-permissive" premise. `max_entries=256` silently implies a different cap from §5.26 rule 3's `id ∈ [0,63]`, creating a semantic gap (validator caps at 64; BPF map allows 256) without explicit reconciliation. If §5.26 cap is NOT lifted in same slice, HASH pays hash-lookup tax for no benefit; if §5.26 cap IS lifted, that's a larger separate decision that needs its own scope (PI-13 inner-shape + T_APPLY_REJECTS_MALFORMED fixture rewrite + design.md schema amendment).
- **User value cycle 1**: per-rule counters (same as Options 2/3/4).
- **Costs**: ~5-15 ns/packet hash-lookup tax for no structural benefit at current cap; documentation overhead to explain why BPF map cap and schema cap diverge; latent silent-inheritance-pattern recurrence as a process precedent.
- **Preserves**: nothing meaningful that Option 2 or 3 doesn't also preserve.
- **Open Qs for this option**: Why HASH at a bounded domain of 64? If the answer is "we plan to lift the cap soon", that's a larger decision that should be scoped explicitly, not silently presumed via `max_entries=256`.

### Recommendation

**Synthesizer recommendation (pre-human-gate): Option 2 — "Sparse-direct-bounded ARRAY" (ARRAY.1 + T.5 S-1 + T.9)** as default if MVP-3.4 ships per-rule counters in this slice. Rationale: (a) it honors the verified §5.26 rule 3 contract byte-for-byte (operator's `id ∈ [0,63]` IS the BPF key); (b) it is verifier-trivial and joins the existing `bump_stat` idiom — minimum reviewer pattern-recognition cost; (c) the 16 KiB total memory cost of "sparse usage of dense array" is rounding error; (d) T.5 S-1 (extend inner-value to carry rule_id) is paid once explicitly rather than hidden behind perpetual intern/sidecar maintenance; (e) it directly answers Open Q #13 in the smallest defensible shape, and the question dissolves cleanly under T.3's reframe (allocation policy P-A → ARRAY mechanically).

**Caveat — Option 2 flips to Option 1 (Honest defer) if EITHER (a) PI-13-3.1 adjudication on the inner-value extension returns VIOLATE rather than additive-PASS (Option 2 then needs to fall back to Option 3, which inflates MVP-3.4 — defer becomes cheaper), OR (b) the human gate confirms team appetite for ≤1.5-cycle slices matches §5.28's recent norm AND no operator is awaiting per-rule visibility right now (both conditions appear to hold today per arch-v2.md and MVP-3.3 retro — defer is the sober call).** Secondary caveat: flip to Option 4 if the human gate has explicit forward-looking appetite (stable rule identity across renumber matters for the planned MVP-3.5 JSON-log slice). Do NOT pick Option 5 without an explicit, separately-scoped §5.26 rule 3 amendment — that path silently violates the shipped operator contract and re-runs the silent-inheritance anti-pattern.

**Human-gate resolution (2026-05-24): Caveat (b) held — defer. Option 1.** Option 2 is the standing default if/when MVP-3.4b re-asks this question.

### Open questions

1. **Is the brief BODY's "id:1, id:5, id:100" framing a typo / casual example, or a substantive proposal to lift §5.26 rule 3?** *Resolved at human gate 2026-05-24: typo.* No cap-lift proposal was raised in this round; the example was a brief-authoring slip.

2. **Should per-rule counters ship in MVP-3.4 or defer to MVP-3.4b?** *Resolved at human gate 2026-05-24: defer.*

3. **PI-13-3.1 adjudication on inner-map-value extension (`__u8 → struct {__u8 present; __u32 rule_id;}`)** — additive PASS or byte-shape break VIOLATE? **Still open** — needed before MVP-3.4b scoping, NOT blocking MVP-3.4 under deferred posture.

4. **Does MVP-3.4's `rules + action_table` BPF skeleton (B.2 partial) REQUIRE inner-map-value extension regardless of whether per-rule counters ship?** **Still open** — question for MVP-3.4 task-brief author. If yes, the PI-13 cost is paid in MVP-3.4 anyway and the defer "saves" less (but still preserves slice cohesion).

5. **Is the 64-rule cap a permanent product contract or a temporary scaffold?** *Resolved at human gate 2026-05-24: permanent.* Matches risk-register line 337 "feature, not defect"; no cap-lift proposal in scope.

6. **Should sidecar JSON for label readability be in scope for cycle 1?** Deferred with the per-rule counter slice — re-enters at MVP-3.4b.

### Hidden assumptions

1. **The brief BODY is the standing adjudicator on prior-round Open Questions.** Architecture-v2.md Hidden Assumption #1 (line 397) explicitly named the silent-inheritance-pattern as a documented failure mode: "round-2 conclusions remain valid unless explicitly revisited" is anti-pattern, and the brief BODY must be re-read as adjudicator at the start of each new round. **This round's brief BODY ITSELF contained a factual contradiction with shipped schema (§5.26 rule 3)** — the synthesizer is the line of defense, and registered the contradiction prominently. If the brief BODY is treated as authoritative without verifying against design.md, every downstream slice inherits the "id ∈ [0, ∞)" hallucination. **Consequence if hidden assumption survives**: HASH-camp wins the round on false premises; team commits to BPF layout (and silent §5.26 cap-lift implication) that the operator contract doesn't authorize.

2. **All three architects assume the inner-map-value-shape question (S-1 vs S-2) is "elsewhere".** HASH and ARRAY both flag it in Open Questions; T.5 surfaces it as the real load-bearing decision. None of the three architect lenses *own* this decision because none was scoped to. **Consequence**: Open Q #13 gets answered but MVP-3.4b stays implementation-blocked because S-1 vs S-2 is unresolved; the resolution produces motion without unblocking if MVP-3.4b re-enters this without addressing PI-13-3.1 first.

3. **The `rules + action_table` skeleton in MVP-3.4 row (line 312) is treated by all architects as independent of per-rule counters.** Probably true mechanically but unverified — if rules + action_table needs inner-value extension regardless, deferral (Option 1) saves less than it appears. **Consequence**: human-gate's defer choice assumes a clean separation that the MVP-3.4 brief author hasn't actually confirmed; tracked as Open Q #4 above.

4. **Counter survival across `apply -f` (Prometheus-counter semantic) is the right default, not a decision.** HASH.1 and ARRAY.1 both adopt it implicitly; T does not contest. But this is an operator-observable semantic: operator might expect counter reset on apply (matching current global `stats` behavior which IS reset). **Consequence if hidden assumption is wrong**: per-rule counters surprise operators with non-reset-on-apply semantic that diverges from existing global stats — observability inconsistency. Deferred with the per-rule slice.

5. **The synthesizer can resolve Open Q #13 without resolving Open Q #9 (MVP-3 horizon: quarter vs year).** T.1 (defer) is a strong contender precisely because of the slice-discipline argument; that argument's weight depends on whether the team optimizes for ≤1.5-cycle slices (quarter horizon — defer wins) or longer cohesive features (year horizon — ship wins). *Human-gate 2026-05-24 implicitly confirmed quarter-horizon posture by picking defer.*

### Per-architect summary

- **architect-HASH** — argued FOR `PERCPU_HASH` keyed by `rule_id`. Selected HASH.1 (vanilla, sparse operator-id key, zero-touch schema), HASH.4 (composite key `{rule_id, axis, verdict}` for richer Prometheus surface), HASH.8 (intern-hybrid: operator-id → __u16 internal_id via loader-side intern table, PERCPU_HASH at BPF surface). Key finding: HASH's natural fit is "sparse keys at flexible cardinality", justified by Cilium production prior art. **Load-bearing weakness: silently inherited the brief BODY's "sparse-permissive id" framing without re-testing against §5.26 schema rule 3** — which actually caps `id ∈ [0,63]`. HASH.1's "zero schema change" argument depends on this miscount.

- **architect-ARRAY** — argued FOR `PERCPU_ARRAY[64]`. Selected ARRAY.1 (status-quo-wide: operator's `id ∈ [0,63]` is the BPF index directly, sparse-usage costs 16 KiB total — rounding error), ARRAY.3 (schema-v2 named-rule allocation, dense internal `rule_id`, human-readable Prometheus labels), ARRAY.6 (two-map shadow: BPF PERCPU_ARRAY + userspace JSON metadata sidecar). Key finding: **explicitly re-tested the brief's "REQUIRES dense" framing against §5.26 and found it wrong** — PERCPU_ARRAY needs dense pre-allocation, not dense usage. Honest accounting of the 16 KiB cost. Justified 64-cap defensively (Katran's 512 dense VIPs as headroom comparison).

- **architect-T (sequential contrarian)** — read both architects, attacked framing. Selected T.1 (DEFER per-rule counters out of MVP-3.4 entirely), T.3 (REFRAME Open Q #13 as "rule_id allocation policy" — map type falls out mechanically; under all viable policies ARRAY is natural at 64-cap), T.5 (the real load-bearing decision is the inner-map-value shape, not the map type). Standalone observations: T.2 (PREMISE-BUST — brief BODY's `id:100` example is factually rejected today), T.4 (Cuckoo on bounded domain is mathematically degenerate), T.6 (migration is 2-3 cycles not 1), T.8 (HASH.8 ≡ ARRAY.6 architectural convergence), T.9 (alias `XDPMF_RULE_COUNTERS_MAX`), T.10 (HASH.4 belongs in MVP-3.5+ not MVP-3.4). Key finding: the question itself is mis-posed; once corrected, ARRAY is mechanical fall-out under every viable allocation policy. **T.1 (defer) adopted by human gate.**
