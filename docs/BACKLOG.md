...
### B31 [HIGH, feature] EtherType as a real L2 match axis — PO decision 2026-05-30
hld-workflow synthesis flagged the brief's EtherType field as neither an axis nor matchable today (datapath IP-gated at `mac_filter.bpf.c:630`); all original lenses substituted L4 "proto" for L2 "EtherType". **PO decided 2026-05-30: EtherType is a genuine L2 match** (e.g. "drop ARP"), NOT loose wording for "IP version" → it needs its own EtherType HASH axis (one `kManagedMaps[]` row + one `populate_all_axes` case + datapath axis lookup), consulted at L2 around the ethertype gate; interacts with the IPv4/IPv6 gate-widening. **Land WITH the IPv6 axis slice** (same ethertype-gate touch); **B20 table-driven refactor is the prerequisite** that slice walks into. Closes the brief's field-list coverage gap. Source: `/home/user/agent-teams-review/runs/hld-mint-l2-mac-filter-20260530084033/` + brief Amendment 2026-05-30.
