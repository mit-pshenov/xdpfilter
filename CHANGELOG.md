...
- **MVP-4.36→4.38 (B43–B45, §5.76–§5.78) — `apply --dry-run`: offline rule-image preview (no version bump).** A `compile()`-driven offline render path lets operators preview the exact map-image an `apply` would install **without touching the kernel** (`PI-mvp-4.38-ZERO-TOUCH`; same parse/validate/iface-reconcile errors + exit codes as live apply).
  - **B43 (§5.76):** extracted the libbpf-free `materialize` + render helpers into `materialize.{hpp,cpp}` + `loader_error.{hpp,cpp}` (pure-MOVE, all bodies byte-identical); NEW offline `dryrun_harness` with a byte-exact golden (`dryrun_image.golden`) and three-way agreement (spec-oracle == golden == impl-render).
  - **B44 (§5.77):** the `apply --dry-run` CLI verb. The dry-run branch sits **above** the kernel-touch flow (the flag lives only in `ApplyConfig`, never `ApplyRequest`), keeping `loader.cpp`/`apply_internal.hpp` byte-identical. Map writes run through a runtime `MapWriter` object-seam (`LiveMapWriter` for apply, `RecordingMapWriter` for dry-run) — wrappers fail closed if no writer is installed.
  - **B45 (§5.78):** the **default** dry-run output is now a human-decoded per-rule view (`format_dryrun_human`); the byte-faithful `# xdpfilter-image v1` machine image moved behind `--format=golden`. `--format` without `--dry-run` is a CLI error.

## [0.16.0] - 2026-06-03

### Changed
- **MVP-4.26 (B33, §5.66) — project-wide rename `xdpmacfilter`/`mac_filter` → `xdpfilter`.** The artifact is a 9-axis L2/L3/L4 bit-vector classifier, not a MAC-only filter; the name now aligns with the already-`xdpfilter_*` Prometheus metrics. Pure mechanical rename + history-preserving `git mv` — NO logic, codegen, map-layout, schema, or env-symbol change (xdp instruction stream stays 3658 insns, verified).
  - **Operator-visible BREAKING surfaces renamed — existing deployments MUST re-create pins + units:**
    - binary `xdpmacfilter` → `xdpfilter`
    - bpffs pin-root `/sys/fs/bpf/xdpmacfilter` → `/sys/fs/bpf/xdpfilter`
    - sidecar-root `/run/xdpmacfilter` → `/run/xdpfilter`
    - systemd unit `xdpmacfilter@.service` → `xdpfilter@.service`
    - Ansible playbook `xdpmacfilter-deploy.yml` → `xdpfilter-deploy.yml`
    - doc dir `share/doc/xdpmacfilter/` → `share/doc/xdpfilter/`
  - **UNCHANGED (no migration needed):** Prometheus metric names (`xdpfilter_*`, already aligned), config `schema_version`, all map names, STAT enumerators, rule/action structs, and the `XDPMF_*` env-var/macro **symbol** names (operator-ABI KEEP; only the embedded path *values* changed; reinterpret `MF` as "Match/Multi-Filter").
...
- **MVP-4.14 (S5, §5.54)** — 9th match axis **`ethertype`** (inner post-VLAN EtherType, exact-match HASH — names `ipv4`/`ipv6`/`arp`, hex `0x…`, or numeric). `BV_AXIS_ETHERTYPE`; `BITVEC_NUM_AXES` 8→9; `kManagedMaps[]` 36→39. Hoisted above the family dispatch and composed into all three arms, incl. a NEW full-symmetric 9-term non-IP `else` arm. **Side effect:** `mac`/`vlan` are now family-blind (fire on non-IP + IPv6 frames), superseding the old IPv4-only MAC gate. `ethertype:arp drop` now works.
- **MVP-4.13 (S4, §5.53)** — IPv6 CIDR axes **`dst_cidr6`/`src_cidr6`** (longest-prefix LPM over `__int128`). `BITVEC_NUM_AXES` 6→8; `kManagedMaps[]` 30→36; symmetric 8-term AND in both family arms; v4 arm verdict-identical. Base-header only (ext-header depth added in S6).

### Changed
- **MVP-4.21 (B30, §5.61)** — **decouple operator `id` from internal `slot`.** The operator `id` is now a sparse stable identity + counter key; the loader assigns a dense `id`-sorted `slot ∈ [0, count)` and shifts `1ULL<<slot`. The kernel raw/counter maps are SLOT-keyed; a NEW userspace-only `slot_rule_id` map (`kManagedMaps[]` 38→39) lets the exporter recover the `id` for the `rule_id` label. The old `id < 64` **value** cap is removed — any `u32` id is legal except the reserved `0xFFFFFFFF` sentinel; the ≤64 limit migrates to a rule-**count** cap. Renumbering/inserting a rule no longer moves its Prometheus counter series. Datapath bytecode **byte-identical** (verified via objdump, 3658 insns).
- **MVP-4.18 (B29, §5.58)** — remove the legacy `allowlist` alias BPF map + its bespoke loader control-flow; `kManagedMaps[]` 39→38. Live datapath verdict-identical.
- **MVP-4.17 (B24+B25, §5.57)** — housekeeping: delete vestigial `classify_match_kind`/`match_kind`; fix stale `config.hpp`/`config.cpp` comments to reflect `schema_version 2` + the 9-axis model.
- **MVP-4.10 (B28)** — templated the 3+3 near-duplicate axis-lowering functions into `populate_hash_inner_slot<Key>` + `aggregate_axis<Key>` (−99 LOC).
- **MVP-4.8 (B20)** — `apply_request` made table-driven (`populate_all_axes` + `inactive_axis_fd`), the prerequisite refactor for the later additive axes.
- **C3 fast-follow** — surface the v6 + ethertype match-kinds on both observability surfaces (sidecar JSON + `rule_info` labels).
- **MVP-4.7 (§5.47)** — MAC-axis return (rule-model S7): the original v1 src-MAC match axis is UN-FROZEN as the 6th exact-match bit-vector axis. The frozen `allowlist` HASH inner value is reshaped `struct allow_entry`→`__u64` rule-bitmask (byte-neutral; pin names + `rulesets` outer UNCHANGED, `kManagedMaps[]` stays 30), the datapath re-consults it (`acc &= (mac_mask|wc_mac)` keyed on `eth->h_source`, EXACT no-closure, AND-composed inside the IPv4 block → matches IPv4 frames only), the `mac` config key is RE-ACCEPTED (canonical 17-char parser re-added; joins the at-least-one-of set), and `xdpfilter_rule_info` gains an 8th `mac` label (LAST). Purely **additive within `schema_version: 2`**. Retires `PI-mvp-4.3-MAC-DEFERRED`. `BITVEC_NUM_AXES` 5→6 (`wildcard` auto-grows 10→12); `BV_AXIS_MAC=5`. VERSION `0.14.0` → `0.15.0`.
- **MVP-4.6 (§5.46)** — `xdpmf-exporter`: NEW additive Prometheus metric family `xdpfilter_rule_info{iface,rule_id,dst_cidr,src_cidr,protocol,dst_port,vlan} 1` (constant-`1` info gauge) makes the 5-axis match model observable — each sidecar-known rule's per-axis constraints surface as labels (empty `""` value for an unconstrained axis), joinable to `xdpfilter_rule_match_total` on `(iface,rule_id)` in PromQL. CONSUMER-ONLY: the exporter reader gains per-axis extraction (key-anchored scan over the existing match-object body, NO JSON-parser dep) and `prom_format` appends the family LAST — the `xdpfilter_packets_total` + `xdpfilter_rule_match_total` blocks stay byte-identical (counter contract preserved). No BPF/datapath/loader/config/schema/map/sidecar-producer change. Retires `PI-mvp-4.3-EXPORTER-AGNOSTIC` (exporter is axis-AWARE by design). VERSION `0.13.0` → `0.14.0`.
- **MVP-4.4 (§5.44)** — ADDITIVE +2 bit-vector match axes (rule-model S4): NEW `protocol` (L4 proto, exact-match HASH — names `tcp`/`udp`/`icmp` or numeric `[0,255]`) and `dst_port` (L4 destination port, integer or `"lo-hi"` inclusive range) match keys extend the §5.43 AND classifier to four axes — `acc = (dmask|wc_dst) & (smask|wc_src) & (proto_mask|wc_proto) & (port_mask|wc_port)`. The datapath gains its first L4 (TCP/UDP) header parse (`ip->ihl*4` L4 offset, bounds-checked; non-TCP/UDP frames carry no port so only port-wildcard rules survive that axis). Purely **additive within `schema_version: 2`** — existing dst/src-only v2 configs parse unchanged. VERSION `0.11.0` → `0.12.0`; `BITVEC_NUM_AXES` 2 → 4 (`wildcard` ARRAY auto-grows 4 → 8); `kManagedMaps[]` 21 → 27 (+`proto_bitmask_a`/`_b`, `proto_rulesets`, `port_ranges_a`/`_b`, `port_rulesets`).
...
### Fixed
- **MVP-4.9 (B18/B19)** — datapath: `port_scan` `continue`→`break` (the inclusive-range scan now stops at the first non-match instead of walking the rest); plus a `build_cpu` `RESOURCE_LOCK` for the `-j4` sanitizer-build-timeout flake (also resolves B16).
...
### Tests / internal
- **MVP-4.20 (B23-min, §5.60)** — test-honesty: reworded the misleading "verifies on the 5.15 floor" over-claim in `T_BITVEC_VERIFIER_LOAD.sh` to the accurate "prototype object loads on the dev kernel" (NOT a 5.15-floor nor a production-object guarantee); behavioral core byte-unchanged.
- **MVP-4.19 (B22, §5.59)** — sanitize the 9-axis lowering path under ASAN/UBSAN (`T_SANITIZER_BUILD` now exercises a full multi-axis config + wildcard + NOMATCH vectors); test-only, ASAN-clean.
- **MVP-4.12 (S2)** — NEW IPv6 packet injector `tests/lib/inject_l6.py` (scapy, L2 `sendp`) + verifying ctest; pure test-tooling (no `src/` change).
- **MVP-4.11 (S1)** — EtherType gate-scaffold: the datapath's family seam becomes `if (ETH_P_IP) / else if (ETH_P_IPV6)` (empty arms → defaults at this step); IPv4 bit-identical. Scaffolding for the S4–S6 axes.
...
### Removed
- **Single `rule_counters` PERCPU_ARRAY pin** — `${PIN_DIR}/<iface>/rule_counters` no longer exists; replaced by `rule_counters_outer` + `rule_counters_a` + `rule_counters_b` pins. The `XDPMF_MAP_RULE_COUNTERS_NAME` constant is removed from `src/common/xdpfilter.h`.
...
- **PI-10-3.4d**: `src/common/xdpfilter.h` ADDITIVE-modulo-deletion (3 added + 1 removed); all other constants/structs/enums byte-equivalent. `XDPMF_RULE_COUNTERS_MAX` alias UNCHANGED (operator-observable index space per §5.31 Q5 R1).
...
- **Explicit drop-rule action dispatch** — `xdpfilter_prog` extends BOTH the MAC HASH-hit AND CIDR LPM_TRIE-hit branches with a 3-step chain: `rules_outer[active]` → `rules_inner[entry->rule_id]` → `action_table[rule.action_id]` → `XDP_PASS` or `XDP_DROP`. Per-rule counter `bump_rule()` runs BEFORE the dispatch chain — counter bumps on every match regardless of verdict (HG-3.4b-c2-5).
...
- **`xdpfilter_prog` datapath body** — MAC HASH-hit + CIDR LPM_TRIE-hit branches grow with the 3-step rules→action_table dispatch chain + XDP_DROP verdict path; 3 new chained `bpf_map_lookup_elem` calls per match. All NULL-checked (verifier-required). Active snapshot discipline preserved — the SAME `active` u32 read at the head of the datapath indexes all 4 outers (MAC + CIDR + defaults + rules).
...
### Removed
- **SHARED `rules` ARRAY pin** — `${PIN_DIR}/<iface>/rules` no longer exists; replaced by `rules_outer` + `rules_a` + `rules_b` pins. The `XDPMF_MAP_RULES_NAME` constant is removed from `src/common/xdpfilter.h`.
- **§5.29 stderr WARN** — `xdpfilter: rules: section parsed (<N> entries) but per-rule action dispatch deferred to MVP-3.4b — datapath uses MAC/CIDR-only matching this cycle` is GONE. The contract it announced no longer applies — datapath consults the chain per HG-3.4b-c2-4.
...
- **PI-10-3.4b-c2**: `src/common/xdpfilter.h` ADDITIVE-modulo-deletion (3 added + 1 removed); all other constants/structs/enums byte-equivalent. STAT_MAX = 4 unchanged (Q1.B re-uses STAT_DROP_DENY for explicit-rule-drops, no new enum slot).
...
- **PI-28-3.4b LIFTED** + **PI-29-3.4b LIFTED** — replaced by PI-29-3.4b-c2 above (`xdpfilter_prog` body extends with the dispatch chain; rules + action_table NOW consulted — the entire purpose of this slice).
...
- **PI-10**: `src/common/xdpfilter.h` UNCHANGED (stricter than its previous ADDITIVE-ONLY baseline — this slice doesn't add to xdpfilter.h at all).
- **PI-28-3.4b**: `xdpfilter.bpf.c` UNCHANGED — JSON logging is userspace-only (D-3.5-10 NEW FENCE).
...
- **PI-10-3.4b**: `src/common/xdpfilter.h` ADDITIVE-ONLY (new `struct allow_entry`, `XDPMF_MAP_RULE_COUNTERS_NAME`, `XDPMF_RULE_COUNTERS_MAX`, `XDPMF_SIDECAR_ROOT` per §5.31 EDIT-1; existing constants + struct layouts UNCHANGED).
- **PI-13-3.4b**: NEW — inner-allowlist-value byte layout documented byte-by-byte; offset-0 byte-equivalence to PI-27 preserved.
- **PI-28-3.4b**: `xdpfilter_prog` body extends with `bump_rule` calls + typed-pointer inner-value reads at MAC HASH-hit and CIDR LPM_TRIE-hit branches. All other body lines byte-equivalent. First substantive body change since MVP-3.2.
...
### Fixed
- HK-1: `xdpfilter apply -f <missing> --iface <X>` now exits **1** (CLI usage error per §4.1) instead of leaking out of the `std::visit` body and tripping the generic `LoadFailed` (exit 2) arm. `main.cpp` SECOND try block gains a `catch (CliError)` arm that mirrors the FIRST try (parser) arm. Existing `xdpfilter: config error:` stderr prefix preserved verbatim; YAML parse / schema-validation failures of a file that DOES exist continue to exit 9 (D-3.4.5-5 rationale: file-IO is upstream of YAML and belongs in usage-error space).
...
- HK-10: `T_LINK_PERSIST_ACROSS_LOADER_EXIT` no longer issues a broad `pkill -9 -f xdpfilter` that could collateral-kill concurrent loaders during parallel ctest runs; the kill is now iface-scoped.
...
### Changed
- HK-2: `xdpfilter --help` exit-code list now includes `7 kernel-unsupported,` between `6 permission,` and `8 path-refused,` (PI-9 forward-compatibility preserved; T_CLI_HELP_VERSION ERE still matches).
- HK-3: `XDPMF_BPF_OBJECT_PATH` env-var override is now compile-gated behind `XDPMF_ENABLE_BPF_OBJECT_OVERRIDE` CMake option (default OFF). Release builds (`-DBUILD_TESTING=OFF`) have the env-var code path `#ifdef`'d out of the binary — `nm $(which xdpfilter) | grep -c XDPMF_BPF_OBJECT_PATH` returns 0. In-tree dev/test builds force the option ON via `tests/CMakeLists.txt`, so `T_VERIFIER_REJECT.sh` continues to pass.
- HK-5: `xdpfilter.bpf.c` adds `__builtin_expect(!!(x), 0)` (`unlikely(x)`) hints on the six verifier-mandated leaf null-checks/bounds-checks. Functional verdicts byte-equivalent (PI-28).
- HK-6: `xdpfilter --help` `--unsafe` line clarified — `--unsafe` is required in non-interactive context AND suppresses the interactive y/N prompt when passed at a tty. New `Environment variables:` sub-block documents `XDPMF_TRUST_MODEL`. `xdpmf-exporter --help` gains an `Environment variables:` block documenting `XDPMF_BPFFS_ROOT` plus a cross-reference note that `XDPMF_TRUST_MODEL` is loader-only.
...
### Added
- HK-7: `docs/FLEET_DEPLOYMENT.md` is now installed under `${CMAKE_INSTALL_PREFIX}/share/doc/xdpfilter/` alongside the systemd units, gated on the same `XDPMF_INSTALL_SYSTEMD_UNIT` option. The two units' `Documentation=file:///usr/share/doc/xdpfilter/FLEET_DEPLOYMENT.md` URI now resolves on the operator's host post-install.
...
- PI-28: `xdpfilter_prog` semantic byte-equivalent; HK-5 only adds compiler branch hints.
...
- `xdpfilter bypass --iface <X> [--unsafe] [--reason "<text>"]` subcommand (§5.29 HG-3.4-2) — operator-facing manual bypass primitive wrapping the existing `loader::detach()` codepath. Interactive tty prompts `y/N`; non-tty REQUIRES `--unsafe` (audit safety gate; exit 1 + recognisable stderr message on absence). ALWAYS emits an audit-log line to stderr BEFORE the detach (D-3.4-5): `xdpfilter: BYPASS activated on <iface> by uid=<UID> reason="<text or UNSPECIFIED>"`. NO new BPF map flag, NO new datapath state (PI-30); the cleanest path that preserves the defer-posture invariants.
- `rules` BPF map (ARRAY[XDPMF_ALLOWLIST_MAX=64] of `struct rule_entry`) + `action_table` BPF map (ARRAY[ACTION_MAX=2] of `struct action_entry`) — DECLARED-AND-POPULATED-NOT-WIRED skeleton per HG-3.4-1. `xdpfilter.bpf.c` datapath body is BYTE-EQUIVALENT to MVP-3.2 modulo the two new `.maps` declarations (PI-28). Populated from `Config.rules` on every `apply` (clear-and-rewrite per D-3.4-8; SHARED — not parallel-swapped — per D-3.4-4 because the datapath does not consult them). Forward-compatibility scaffold for MVP-3.4b wiring once PI-13-3.1 adjudication on the inner-allowlist-value extension lands.
- `apply` orchestrator emits a one-shot stderr WARN when `Config.rules` is non-empty: `xdpfilter: rules: section parsed (<N> entries) but per-rule action dispatch deferred to MVP-3.4b — datapath uses MAC/CIDR-only matching this cycle`. Operator-facing signature of PI-29.
- `src/common/xdpfilter.h` additive types: `struct rule_entry`, `struct action_entry`, `enum xdpmf_action_type` (`ACTION_PASS=0`, `ACTION_DROP=1`, `ACTION_MAX=2`), `XDPMF_MAP_RULES_NAME`, `XDPMF_MAP_ACTION_TABLE_NAME` (PI-10-3.4 strengthened: pure additive diff; existing constants/structs/enum values UNCHANGED).
...
- `CMake install()` rule: both `xdpfilter` and `xdpmf-exporter` now install to `${CMAKE_INSTALL_BINDIR}` (single `install(TARGETS …)` line). The `XDPMF_INSTALL_SYSTEMD_UNIT` rule now also installs `systemd/xdpmf-exporter.service` alongside the existing template unit.
...
- PI-28: `xdpfilter_prog` BPF function body BYTE-EQUIVALENT to MVP-3.2 modulo two new `.maps` declarations (`rules` ARRAY + `action_table` ARRAY). Datapath does NOT issue `bpf_map_lookup_elem` against either new map.
...
- PI-33: `xdpfilter --version` AND `xdpmf-exporter --version` BOTH report `0.6.0` (shared `version.h` from CMake `project(VERSION)`).
...
- `docs/FLEET_DEPLOYMENT.md` — operator docs (~95 lines) per Q3 D1 covering: `XDPMF_TRUST_MODEL` decision matrix, audit-log story citing the exact stderr prefix `xdpfilter: trust_model=` (PI-23 verbatim), systemd Drop-In recipe for fleet posture, Prometheus alert semantic (fleet-wide trust-model uniformity; exporter implementation deferred to MVP-3.4), and the §5.4/§5.19/§5.22/§5.24 fence callout (fleet relaxes ONLY §5.4).
...
- PI-8-3.3: `xdpfilter --version` reports `xdpfilter 0.5.0` (driven by CMake `project(VERSION)` via `include/version.h.in` per §5.25 P3).
- PI-9: `--help` / `--version` output FORMAT unchanged (no new CLI flag per D-3.3-1).
- PI-10-3.2: `src/common/xdpfilter.h` constants + struct layout UNCHANGED (strengthened — this slice adds zero new constants).
...
- PI-23: `docs/FLEET_DEPLOYMENT.md` cites the EXACT runtime stderr prefix `xdpfilter: trust_model=` (verbatim, not paraphrased).
- PI-24: unit file directive set EXACTLY matches the §5.28 PI-24 catalogue.

### Out-of-scope fences (per §5.28)
- Binary rename `xdpfilter` → `xdpfilter` — still MVP-3.12 (transitional alias to come).
...
- `xdpfilter.bpf.c` datapath extended with the OR-compose branch: MAC HASH first (O(1) short-circuit per Q2 OR1), then on IPv4 ethertype lookup src_ip in the CIDR LPM_TRIE (O(prefix-length)). Non-IPv4 frames (ARP, IPv6, VLAN-tagged) bypass the CIDR branch entirely — preserves MVP-3.1 semantic for non-IP traffic. IP-header bounds check is verifier-mandatory before `ip->saddr` deref; truncated IPv4 frames bump `STAT_DROP_MALFORMED` (consistent with §5.5).
- `src/common/xdpfilter.h` gains `struct xdpmf_cidr_v4 {prefixlen, addr}` (LPM_TRIE key shape) + 3 new map-name macros (`XDPMF_MAP_CIDR_{RULESETS_OUTER,INNER_A,INNER_B}_NAME`) + `STAT_PASS_CIDR = 3` enum value + `STAT_MAX = 4` bump. Existing constants UNCHANGED.
...
- PI-10-3.2: existing `xdpfilter.h` constants + `struct xdpmf_mac` layout + enum slots 0/1/2 BYTE-IDENTICAL.
...
### Added
- `apply` subcommand: `xdpfilter apply --iface <IFNAME> -f <PATH> [--mode <M>]`. Reads YAML config; atomic hot-swap when an existing link pin is present (no packet drop window; verified by `T_APPLY_ATOMIC_SWAP_NO_DROP`).
- Custom YAML 1.2 subset parser in `src/lib/yaml_subset.{cpp,hpp}` (no third-party deps per HG1). Accepted constructs: block mapping, block sequence, single/double-quoted scalars, bareword scalars, signed-decimal integers, null/`~`, `#` comments, optional leading `---`. DoS guards: 1 MiB file, 4 KiB scalar, 8-level nesting. Everything else → `ConfigError` (exit 9) with `xdpfilter: config error: <feature>: <file>:<line>:<col>` stderr.
...
- `XDPMF_TRUST_MODEL` env var (HG3): `strict` (default) | `fleet`. `fleet` relaxes the §5.4 alien-program refusal ONLY (still detaches the alien and proceeds). `§5.19` (name-check) and `§5.22` (tag-check + O_PATH path-discipline) remain enforced in both modes. Audit story: mandatory stderr-log `xdpfilter: trust_model=<m>` at `attach`/`apply` entry.
...
- `xdpfilter.bpf.c` datapath rewritten to use the chained `active_idx` → `rulesets[active]` → inner MAC lookup → `defaults[active]` pattern. `xdpfilter_prog` symbol name + SEC unchanged (§5.19 / §5.22 identity-gate contract preserved).
...
- PI-8: `xdpfilter --version` reports `xdpfilter 0.3.0` (CMake `project(VERSION)`-driven via `include/version.h.in`).
...
- `xdpfilter --version` output now reflects `project(VERSION)` (was hardcoded `0.1.0`; is now `0.2.3` and tracks the CMake version automatically going forward).
...
### Added
- Kernel-version probe via `uname(2)` + parse `release` field; floor is `5.15`. Fires at the head of both `attach()` and `detach()` BEFORE any libbpf call — replaces cryptic `BPF_PROG_LOAD: Invalid argument` from deep libbpf with a clear `xdpfilter: kernel <maj>.<min> too old, need ≥ 5.15`.
...
- `T_VERIFIER_REJECT` — regression test for the verifier-reject path. Uses a deliberately-bad BPF fixture (`tests/fixtures/xdpfilter_bad.bpf.c` — unbounded loop without `#pragma unroll`); asserts loader exits 2 (`LoadFailed`) with recognizable libbpf stderr. Degrades gracefully (`SKIP_RETURN_CODE 77`) if the running kernel happens to accept the bad fixture.
...
- §5.22 O_PATH bpffs root hardening: `BpffsRootFd` RAII opens `/sys/fs/bpf/xdpfilter/` with `O_PATH | O_DIRECTORY | O_NOFOLLOW`; all bpffs ops converted to fd-relative `*at()` syscalls (`faccessat`/`mkdirat`/`openat`+`fdopendir`+`unlinkat` walk/`fstatat AT_SYMLINK_NOFOLLOW`). Closes the symlink-vortex vector at both root and per-iface levels.
...
- `tests/fixtures/xdpfilter_alt.bpf.c` — alt BPF fixture (same SEC name, minimal body) for the tag-mismatch test.

### Changed
- `detach()` is now symmetric to `attach()` for the identity gate: also early-loads skeleton, captures self_tag, runs the probe — closes a parallel attacker-recompile vector where an attacker's planted same-named alien could be detached by an operator running `xdpfilter detach`.
...
- Cross-loader idempotency is NOT supported: if you manually load `xdpfilter.bpf.o` via `bpftool prog load` or `ip link set xdpgeneric obj` outside of `xdpfilter`, the loader will refuse to recognize it as ours (exit 4 + `tag mismatch`). Reason: kernel-computed `bpf_prog_info.tag` differs across libbpf rewrite paths (CO-RE relocations, subprog inlining) even for the same `.bpf.o`. Workaround: detach the external load first (`ip link set <iface> xdp off`), then run `xdpfilter attach`.
...
- §5.19 identity-verified ownership: `bpf_prog_info.name` match against
  `"xdpfilter_prog"` gates the "ours" classification.
...
### Added
- XDP program (`src/bpf/xdpfilter.bpf.c`): L2 source-MAC allow-list
  with `XDP_PASS` / `XDP_DROP` decisions; three pinned counters
  (`STAT_PASS`, `STAT_DROP_DENY`, `STAT_DROP_MALFORMED`).
- C++23 loader (`xdpfilter`) with `attach` / `detach` subcommands;
  per-iface bpffs pin layout at `/sys/fs/bpf/xdpfilter/<iface>/`;
